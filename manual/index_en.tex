...
\pagestyle{empty}
\usepackage{oao378}
\title{Document Server System Manual}
\author{devteam}
\date{\today}
\begin{document}
...
In this document, overall conceptual design and features of the document 
server system is shown.
Since some technical terms are used as definition specified to this system, 
please refer terminology definitions before using and refering this document.
...
\begin{description}
  \item[File ID] 各実体ファイルに割り当てられている22文字によるコード
\end{description}
...
ホームで表示する画面は、ユーザ設定で{\tt home}でURLにより指定できます。
...
\begin{description}
  ...
  \item[history] homeを{\tt index.cgi}にした場合に表示されるホームでの更新履歴
...
データベース定義SQLファイルが{\tt common.sql}として提供されているので、
...
\subsubsection{{\tt cl\_add\_file.pl}}
...
メール送信機能を利用する場合、{\tt bugmail}属性をメールアドレスとして利用して
